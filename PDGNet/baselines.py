...
def pa_orthog_imperf(q_rate_real, q_rate_est, total_delay, total_energy, d_requirement, e_requirement, data_number, user_number, RB_number):

    # Set value for each adge according to our equation (24) from [1]
    W = np.array(data_number[:q_rate_est.shape[0]]).reshape(-1,1) * (q_rate_est-1)
    W *= (total_delay < d_requirement) & (total_energy < e_requirement)
    ...
    vInd = assignment>=0
    finalq=np.ones(user_number);
    finalq[vInd] = q_rate_real[vInd, assignment[vInd]]
